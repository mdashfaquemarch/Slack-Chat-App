...
function isAdmin(workspace, userId) {
  const isValidAdmin = workspace.members.find(
    (member) =>
...
async function createWorkspaceService(workspaceData) {
  try {
    ...
    const workspace = await workspaceRepo.addChannelToWorkspace(
      response._id,
      'general'
    );
...
async function updateWorkspaceService(workspaceId, workspaceData, userId) {
  try {
    ...
    if (!isValidAdmin) {
      throw new AppError(
        'User is not an admin of workspace',
        StatusCodes.UNAUTHORIZED
      );
    }
...
async function addMemberToWorkspaceService(
  ...
  try {
    ...
    const isValidUser = await userRepo.getById(memberId);
    ...
    const response = await workspaceRepo.addMemberToWorkspace(
      workspaceId,
      memberId,
      role
    );
...
async function addChannelToWorkspaceService(workspaceId, channelName, userId) {
  try {
    ...
    const isChannelPresent = isChannelAlreadyPartOfWorkspace(
      workspace,
      channelName
    );
